...
class MY_GUI():
    ...
    def set_init_window(self):
        ...
        self.log_data_Text = Text(self.init_window_name, width=45, height=35)  # 日志框
        self.log_data_Text.grid(row=11, column=1, rowspan=2,columnspan=2)
        #图形
        fig = Figure(figsize=(5, 4), dpi=100)
        self.ax = fig.add_subplot(111)


        self.canvas = FigureCanvasTkAgg(fig, master=self.init_window_name)

        self.canvas.get_tk_widget().grid(row=11, column=3, rowspan=3, columnspan=2)
        self.canvas._tkcanvas.grid(row=11, column=3, rowspan=3, columnspan=2)
        ...
        self.picture=Button(self.init_window_name, text='重画', command=self.draw).grid(row=15, column=4)
        self.clear=Button(self.init_window_name, text='退出', command=self._quit).grid(row=1, column=5)
    ...
    def _quit(self):
        '''退出'''
        self.init_window_name.quit()  # 停止 mainloop
        self.init_window_name.destroy()  # 销毁所有部件
